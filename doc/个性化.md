...
- 字符串字面量 `String` - `Invalid`、`Valid`和 `String text`（归为一类，`Foreground`都变更为 `#B578B8`）

- 数字字面量 `Number`（`Foreground`都变更为 `#B578B8`）
...
- 字符串字面量 `String` - `Invalid`、`Valid`和 `String text` - `Use inherited attributes`都取消勾选（消除冲突）
